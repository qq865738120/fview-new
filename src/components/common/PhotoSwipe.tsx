...
export default class PhotoSwipeWap extends React.PureComponent<PhotoSwipeProps, any> {
  gallery: any;
  constructor(props) {
    super(props);
    this.state = {
  ...
  init = props => {
    ...
    if (props.items && props.items.length > 0) {
      ...
      gallery.init();
      if (items[props.index || 0].h * items[props.index || 0].initialZoomLevel > gallery.viewportSize.y - 100) {
        gallery.zoomTo(
      ...
      gallery.listen("parseVerticalMargin", item => {
        item.vGap.top = 50;
        item.vGap.bottom = 50;
      });
            ...
            {(items || []).length > 1 && (
              <div style={{ ...indicatorStyle }}>
                {(items || []).map((item, index) => (
                  <span
                    key={index}
                    style={{ ...indicatorItemStyle, backgroundColor: index === indicatorIndx ? "#dcdada" : "#909090" }}
                  />
                ))}
              </div>
            )}
